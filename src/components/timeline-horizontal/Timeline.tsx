...
const Timeline = ({ stories }: TimelineProps) => {
  ...
  const generateFillPositions = (length: number): number[] => {
    ...
    return Array.from(
      { length: numSequences * sequence.length },
      (_, i) => sequence[i % sequence.length]
    );
  };
  ...
  return (
    ...
    >
      ...
      {stories
        .slice(startDisplayIndex, startDisplayIndex + 3)
      ...
      {stories
        ...
        .map((_: any, index: number) => (
          <Box
            ...
            bgcolor={
              index === currentStoryIndex ? 'text.secondary' : 'primary.main'
            }
            borderRadius='50%'
            width={24}
            height={24}
