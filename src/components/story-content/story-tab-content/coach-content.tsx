...
export const CoachContent = ({ story }: StoryContentProps) => {
  ...
  return (
    <StorySectionWrapper title='Izbornik'>
      <Box
        sx={{ display: 'flex', flexDirection: ['column', 'row'], gap: '1rem' }}
      >
        <Box
          sx={{
            flex: 1,
        ...
        >
          ...
          <Typography
            ...
            mb={2}
          >
            <strong>Glavna postignuća:</strong>{' '}
            {story?.Coach.MajorAchievements.join(', ')}
          </Typography>
        </Box>
        <Box
          sx={{
        ...
        >
          <img
            src={story?.Coach.imgUrl}
            alt={story?.Coach.Name}
            style={{ width: '100%' }}
          />
        </Box>
      </Box>
