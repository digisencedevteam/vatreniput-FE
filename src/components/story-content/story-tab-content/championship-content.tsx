        ...
        <>
            {story?.Championship?.Summary && (
                <StorySectionWrapper title='O prvenstvu'>
                    <Typography variant="body1" mt={2}>{story?.Championship.Summary}</Typography>
                </StorySectionWrapper>
            )}
            {story?.Championship?.Summary && (
                <StorySectionWrapper title='Group' isCollapsable={true}>
                    <MatchTable data={story?.Championship.Group} />
                    {story?.Championship?.Matches.GroupStage && (
                        <QualificationMatchDetails matches={story?.Championship?.Matches.GroupStage.matches || []} />
                    )}
                </StorySectionWrapper>
            )}
            {story?.Championship?.Matches?.Finals?.RoundOf16 &&
                <StorySectionWrapper title='Osmina Finala' isCollapsable={true}>
                    <MatchDetails matchData={story?.Championship.Matches.Finals.RoundOf16} />
                </StorySectionWrapper>
            }
            {story?.Championship?.Matches?.Finals?.QuarterFinal &&
                <StorySectionWrapper title='Četvrt Finala' isCollapsable={true}>
                    <MatchDetails matchData={story?.Championship.Matches.Finals.QuarterFinal} />
                </StorySectionWrapper>
            }
            {story?.Championship?.Matches.Finals?.SemiFinal &&
                <StorySectionWrapper title='Polufinale' >
                    <MatchDetails matchData={story?.Championship.Matches.Finals.SemiFinal} />
                </StorySectionWrapper>
            }
            ...
            {
                story?.Championship?.Champ && story?.Championship?.Champ.Winner && story?.Championship?.Champ.TopScorer &&
